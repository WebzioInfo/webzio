...
const Services: React.FC = () => {
  ...
  return (
    <section
      id="services"
      className="py-20 relative overflow-hidden bg-gradient-to-br from-[#F4F3DC] to-[#EDE9DD]"
    >
      {/* Background Glow */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -left-40 w-80 h-80 rounded-full opacity-10 animate-pulse bg-[#2D151F]" />
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Section Header */}
        <div className="text-center mb-16" data-aos="fade-down">
          <h2 className="text-4xl md:text-5xl font-bold mb-6 mt-6 text-[#2D151F]">
            Digital Services for Every Need
          </h2>
          <p className="text-xl max-w-3xl mx-auto text-[#2D151F]/80">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div
              ...
              className={`group p-8 rounded-2xl border transition-transform transform duration-600 ease-in-out hover:scale-[102%] hover:shadow-2xl ${
                index % 2 === 0
                  ? "bg-[#F4F3DC] border-[#2D151F]/20"
                  : "bg-[#EDE9DD] border-[#2D151F]/10"
              }`}
            >
              {/* Icon */}
              <div className="inline-flex p-4 rounded-xl mb-6 bg-[#2D151F]/10 text-[#2D151F] transition-all duration-300 group-hover:bg-[#C1785A]/10 group-hover:shadow-lg">
                {service.icon}
              </div>

              {/* Title */}
              <h3 className="text-2xl font-bold mb-4 text-[#2D151F]">
                {service.title}
              </h3>

              {/* Description */}
              <p className="text-base mb-6 leading-relaxed text-[#2D151F]/80">
                {service.description}
              </p>
              ...
              <ul className="space-y-2">
                {service.features.map((feature, featureIndex) => (
                  <li
                    key={featureIndex}
                    className="flex items-center text-sm text-[#2D151F]/70"
                  >
                    <div className="w-2 h-2 rounded-full mr-3 bg-[#2D151F]" />
                    {feature}
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button className="text-sm font-semibold text-[#2D151F] hover:text-[#C1785A] transition-colors duration-200">
                  Learn More →
                </button>
              </div>
            </div>
        ...
        <div className="text-center mt-16" data-aos="fade-up">
          <p className="text-lg mb-6 text-[#2D151F]/80">
            Don’t see what you’re looking for? We also build custom solutions.
          </p>
          <button className="px-8 py-4 rounded-full font-semibold text-lg bg-[#2D151F] text-[#F4F3DC] hover:bg-[#3A1C28] transition-all duration-300 hover:scale-105 hover:shadow-xl">
